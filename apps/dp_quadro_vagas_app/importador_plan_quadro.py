...
from apps.dp_quadro_vagas_app.models import Vagas_adicionais
from proj_portal_operacional.settings import BASE_DIR


class Importador_Plan_Quadro():
    obj_arquivo = None
    arquivo_quadro = None
    uploaded_file_url = None
    desc_layout_arq = ''
    def __init__(self, arquivo_quadro, desc_layout_arq, obj_arquivo):
        data_hora_atual = datetime.now()
        data_atual_dd_mm_yyyy = data_hora_atual.strftime('%d/%m/%Y')
        hota_atual = data_hora_atual.strftime('%H:%M:%S')

        caminho_arq_importado = 'docs/plan_quadro_freightech/' + str(arquivo_quadro.name).replace('.xlsx', '') + '_' + \
                                str(data_atual_dd_mm_yyyy).replace('/', '_') \
                                + '_' + str(hota_atual).replace(':', '_') + '.xlsx'
        fs = FileSystemStorage()
        filename = fs.save(caminho_arq_importado, arquivo_quadro)
