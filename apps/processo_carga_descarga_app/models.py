...
class Despesas_Carga_Descarga_Cliente(models.Model):
    cod_cliente = models.IntegerField(primary_key=True, blank=False)
    cod_promax_cliente = models.CharField(max_length=50, null=False)
    ...
    class Meta():
        managed = True
        db_table = 'op_proc_carga_descarga_cliente'
...
class Despesas_Carga_Descarga(models.Model):
    ...
    cod_cliente = models.ForeignKey(Despesas_Carga_Descarga_Cliente, models.DO_NOTHING, db_column='cod_cliente')
    ...
    comprovante = models.FileField(upload_to=lambda instance, filename: instance.caminho_upload_personalizado(filename), storage=OverwriteStorage(), blank=False, null=False)
    cod_filial = models.ForeignKey(Filial, models.DO_NOTHING, db_column='cod_filial')
    ...
    def caminho_upload_personalizado(self, filename):
        ...
        novo_nome = f"{self.cod_filial.cod_promax}_{self.mapa}_{self.cod_cliente.cod_cliente}{extensao}"
        return os.path.join('docs', 'processo_carga_descarga_app',str(self.cod_filial.cod_promax),str(self.mapa),str(self.cod_cliente.cod_cliente),novo_nome)
