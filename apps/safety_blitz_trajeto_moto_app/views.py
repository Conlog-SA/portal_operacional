...
class Form_Gerar_Check_Blitz_Trajeto_Moto(View):
    @csrf_exempt
    def get(self, request):
        ...
        if colaborador.perfil_usu == 'G':
            ...
            check_ativo = Libera_Filial_Check.objects.filter(cod_check__tipo_check=5,
    ...
    def post(self, request):
        ...
        data_atual = datetime.now()
        check_ativo = Libera_Filial_Check.objects.filter(cod_check__tipo_check=5, cod_filial=colaborador.cod_filial,
                                                         cod_check__data_desativacao__gte=date(data_atual.year,
                                                                                               data_atual.month,
                                                                                               data_atual.day),
                                                         cod_check__data_inicio__lte=date(data_atual.year,
