...
$(document).on('click','button', function(){
    ...
    // Coleta de dados da form do cadastro de equipamentos e veiculos
        ...
        let let_cod_anexo = let_value_btn;
          $.ajax({
            type: 'DELETE',
            url: '/cco_multas_app/salva_anexo_delete/' + let_cod_anexo,
            dataType: 'json',
            data: {
                'cod_anexo_multa' : let_cod_anexo
            },
            success: function(data){
                $.gritter.add({
                    title: 'Atenção!',
     ...
     success: function(data) {
        let let_visualiza_tabela = data.lista_anexos_multa.map(linha => [
            ...
            <button type='button' id="btn_excluir_anexo"
                ...
                title="Excluir Anexo">
                <i class="fa-solid fa-trash-can" style="background-color: #f46424";></i>
            </button>
            `
            ])
