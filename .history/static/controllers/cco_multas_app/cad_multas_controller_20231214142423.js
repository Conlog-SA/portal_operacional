...
$(document).on('click','button', function(){
    ...
    if (let_name_btn == 'btn_pesquisa_status'){
        ...
        let let_tipo_pesquisa = $("#hidden_status_multa").val()
        $.ajax({
        type : 'DELETE',
        ...
        success: function(data){
            $.gritter.add({
                ...
                atualiza_placa_multa(let_cod_multa);
                atualiza_tab_pesquisa_multa(let_tipo_pesquisa);
            },
            error: function (request, status, error) {
                $.gritter.add({
                    title: 'Atenção!',
    ...
    // Coleta de dados da form do cadastro de equipamentos e veiculos
        ...
        let let_cod_anexo = let_value_btn;
           $.ajax({
                ...
                success: function(data){
                    ...
                    $("#div_visualizacao_anexo_multa").html("");
                },
                error: function(request, status, error){
                    $.gritter.add({
                        title: 'Atenção!',
        ...
        let_caminho_multa = let_value_btn;
        ...
        let_img_anexo_selecionado.attr({
            ...
            src: '/media/'+let_caminho_multa,
            width: '430px',
            height: '800px',
            title: 'Clique na imagem para imprimir o documento'
        });   
        $("#div_visualizacao_anexo_multa")
         .html('<embed  src="/media/'+let_caminho_multa+'" style="width:100%; height:1000px;"/>');
    }
...
function atualiza_tab_pesquisa_multa (tipo_pesquisa){
    ...
    if(tipo_pesquisa =='placa'){
       ...
       dados_parametros = {
        ...
        status_selecionado = $("#select_status_multa").val()
        dados_parametros = {
            csrfmiddlewaretoken: $("[name=csrfmiddlewaretoken]").val(),
    ...
    $.ajax({
        ...
        data: dados_parametros,
        success: function(data) {
            $("#hidden_status_multa").val(tipo_pesquisa);
...
function atualiza_placa_multa (placa){
    $("#select_placa_multa option").remove();
    $.ajax({
        type: "GET",
        dataType: "json",
        url: "/cco_multas_app/atualiza_placa",
        success: function(dados){
            dados.lista_placas.forEach(placa => {
                $("#select_placa_multa").append("<option value='"+
...
function atualiza_data_atual(data){
    let let_get_date = document.getElementById("select_dt_cad_cco");
    let let_current_date = new Date(). toISOString().split('T')[0];
    let_get_date.value = let_current_date;
}
