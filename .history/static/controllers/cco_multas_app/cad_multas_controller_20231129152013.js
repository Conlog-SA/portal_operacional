...
$(document).on('click','button', function(){
    ...
    if (let_name_btn == 'btn_pesq_placa'){
        let let_pesq_placa = 'btn_pesq_placa';
        if (let_value_btn == '0'){
            $.gritter.add({
                title: 'Atenção!',
                text: 'Procure por uma placa',
        ...
        success: function(data){
            $.gritter.add({
                ...
                atualiza_tab_pesquisa_multa(data.cod_multa_antt);
            },
            error: function (request, status, error) {
                $.gritter.add({
                    title: 'Atenção!',
    ...
    // Coleta de dados da form do cadastro de equipamentos e veiculos
        ...
        $("#div_visualizacao_anexo_multa")
         .html('<embed  src="/media/'+let_caminho_multa+'" style="width:100%; height:100%;"/>');
    }
...
function atualiza_tab_pesquisa_multa (tipo_pesquisa){
    ...
    if(tipo_pesquisa =='placa'){
       ...
       dados_parametros = {
        ...
        status_selecionado = $("#hidden_status_multa").val()
        dados_parametros = {
            csrfmiddlewaretoken: $("[name=csrfmiddlewaretoken]").val(),
