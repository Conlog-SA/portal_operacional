...
$(document).on('click','button', function(){
    ...
    //Botão ao clicar para desativar a punição na tabela abre o modal.
    if (let_name_btn == 'btn_abre_modal_fluxo_punitivo'){
        let let_cod_punicao = let_value_btn;
        $("#hidden_cod_punicao").val(let_cod_punicao)
        $("#modal_desativa_punicao").show();
    }
    else if (let_name_btn == 'btn_cancela_modal_desativa_fluxo'){
        $("#modal_desativa_punicao").hide();
    }
    else if (let_name_btn == 'btn_fecha_modal_desativa_fluxo'){
        $("#modal_desativa_punicao").hide();
    }
    //Botão ao clicar cadastra a desativação
    else if (let_name_btn == 'btn_modal_cria_novo_desativacao'){
        let let_cod_punicao = $("#hidden_cod_punicao").val();
        let let_desativado = 'N'
        let let_motivo_desativacao  = $("#input_desc_desativacao").val();
        let filial = $("#btn_abre_modal_fluxo_punitivo").data('filial');
        let competencia = $("#btn_abre_modal_fluxo_punitivo").data('competencia');
        if (let_desativado === 'N'){


            $.ajax({
                type: "POST",
                url: "/gente_gestao_fluxo_punitivo/cancela_punicao",
                data: {
                    "cod_punicao":  let_cod_punicao,
                    "ativo": let_desativado,
                    "obs_punicao": let_motivo_desativacao
                },
                success: function(data) {
                    $("#modal_desativa_punicao").hide();
                    atualizarTabela(filial,competencia);
                    $.gritter.add({
                        title: 'Sucesso!',
                        text: "Punição Desativada com Sucesso!",
...
function atualizarTabela(filial,competencia) {
        ...
        success: function(data) {
            ...
            data.linhas_tabela.forEach( linha => {
                let let_linha = [
                    ...
                    `
                    <button type='button'
                        id="btn_abre_modal_fluxo_punitivo"
                        name="btn_abre_modal_fluxo_punitivo"
                        class="btn btn-primary btn-rounded icon-desativado"
                        value="${linha.cod_punicao}"
                        data-filial="${filialPesquisada}"
                        data-competencia="${competenciaPesquisa}"
            ...
            $('#btn_desativa_fluxo_punitivo').on('click', function() {
                $("#modal_desativa_punicao").show();
            });
...
$('#form_pesquisa_fluxo_punitivo_tabela').submit(function(event) {
    event.preventDefault();
    atualizarTabela(); // Chama a função para atualizar a tabela
});
