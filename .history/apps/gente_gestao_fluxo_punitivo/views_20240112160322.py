...
from apps.gente_gestao_fluxo_punitivo.models import Gente_Gestao_Motivo_Juridico, Gente_Gestao_Motivo_Especifico, \
    Gente_Gestao_Punicao, Gente_Gestao_Dias_Suspensao, Gente_Gestao_Desativacao
...
class Cancela_Punicao(View):
    def post(self, request):
        cod_punicao = request.GET['cod_punicao']
        motivo_cancelamento_punicao = request.GET['obs_cancelamento_punicao']

        if cod_punicao is not None :
            obj_punicao = Gente_Gestao_Punicao.objects.get(pk=cod_punicao)

            obj_desativacao = Gente_Gestao_Desativacao(
                motivo_desativacao = motivo_cancelamento_punicao,
               data_desativacao = timezone.now()
            )

            obj_punicao.save()
            obj_desativacao.save()
