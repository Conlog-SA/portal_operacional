...
class Tabelaa_Fluxos(View):
    ...
    def post(self,request):
        cod_desativacao = request.POST["cod_punicao"]
        motivo_desativacao = request.POST["motivo_desativacao"]
        cod_usu_desativou = request.session["cod_usuario_logado"]

        if cod_desativacao is not None :
            desativacao  = Gente_Gestao_Desativacao(
            motivo_desativacao = motivo_desativacao
            cod_usu_desativou = cod_usu_desativou
            )
            desativacao.save()
