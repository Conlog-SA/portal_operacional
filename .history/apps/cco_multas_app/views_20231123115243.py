...
class Anexa_Doc_View(View):
    def post(self, request):
        ...
        caminho_arquivo = 'docs/cco_multas_app/'+ tipo_anexo + '_' + str(cod_multa) + '.pdf'
        filename = fs.save(os.path.join(caminho_arquivo, file_form.name), file_form)
...
class Pesquisa_Anexo_View(View):
    def get(self,request):        
        ...
        obj_multa = CCO_Multas.objects.get(pk=pk)
        lista_anexos_multa = list(CCO_Anexos.objects.filter(cod_multa_antt=obj_multa).values('cod_multa_antt__placa_multa','cod_anexo_cco','tipo_anexo',))
