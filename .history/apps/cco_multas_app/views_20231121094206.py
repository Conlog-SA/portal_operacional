...
def anexar_itens(request):
    if request.method == 'POST':
        notificacao_file = request.FILES.get('file_anexo_notificacao')
        multa_file = request.FILES.get('file_anexo_multa')

        if notificacao_file and multa_file:
            # Validar tipos de arquivo (exemplo: permitir apenas PDF)
            if notificacao_file.name.endswith('.pdf') and multa_file.name.endswith('.pdf'):
                # Caminhos dinâmicos
                notificacao_path = os.path.join(settings.MEDIA_ROOT, 'notificacoes', notificacao_file.name)
                multa_path = os.path.join(settings.MEDIA_ROOT, 'multas', multa_file.name)

                # Salvando os arquivos
                with open(notificacao_path, 'wb+') as destination:
                    for chunk in notificacao_file.chunks():
                        destination.write(chunk)

                with open(multa_path, 'wb+') as destination:
                    for chunk in multa_file.chunks():
                        destination.write(chunk)

                return JsonResponse({'message': 'Arquivos recebidos com sucesso'})
            else:
                return JsonResponse({'error': 'Tipos de arquivo inválidos. Apenas PDFs são permitidos.'}, status=400)
